...
model2 = load_model("model_ResNet502.h5")
...
def Predict_dog_cat1(filenames):
    preds=[]
    for filename in filenames:   
...
def functional():
    ...
    # Zipp=zip(file_names,preds,prc1,prc2)
    preds= Predict_dog_cat1(file_names)
...
def Predict_dog_cat3(filenames):
    preds=[]
    for filename in filenames:   
        img = image.load_img("static/uploads/"+ filename ,target_size = (150,150))
        x = image.img_to_array(img)
...
def dog_vs_cat():
    ...
    flash('Image est charger par succes')
    preds= Predict_dog_cat3(file_names)
    Zipp=zip(file_names,preds)
